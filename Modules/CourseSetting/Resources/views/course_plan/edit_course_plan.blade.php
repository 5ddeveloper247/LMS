...
@section('mainContent')
    @php
        // $course_type = $course_plan->type;
        // if ($course_type == 'full_course') {
        //     $d_none = 'd-block';
        // } elseif ($course_type == 'prep_course_live') {
        //     $d_none = 'd-block';
        // } else {
        //     $d_none = 'd-none';
        // }
    ...
    <section class="admin-visitor-area up_st_admin_visitor">
        <div class="white_box mb_30 student-details header-menu">
            <div class="white_box_tittle list_header">
                <h4>{{ __('Edit Course') }} {{ __('Plan') }} </h4>
            </div>
            <input type="hidden" id="url" value="{{ url('/') }}">
            <form action="{{ route('updateCoursePlan') }}" method="POST" enctype="multipart/form-data" id="course_plan_form">
                @csrf
                <input type="hidden" name="payment_plan_id" value="{{ $course_plan->id }}">
                <div class="row">
                            ...
                            <select class="primary_select disabled" name="course_id" id="course_id"
                                ...
                                @foreach ($courses as $course)
                                    <option value="{{ $course->id }}"
                                        {{ $course->id == $course_plan->courses->parent_id ? 'selected' : '' }}>
                                        {{ $course->title }} </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="row mt-4" id="child_course_section">
                    <div class="col-xl-6">
                        @if ($course_plan->type == 'full_course')
                            <label class="primary_checkbox nowrap d-flex mr-12">
                                <input type="radio" name="full_course" value="{{ $course_plan->parent_id }}"
                                    id="full_course" class="child_course"
                                    {{ $course_plan->type == 'full_course' ? 'checked' : '' }}>
                                <span class="checkmark mr-2"></span>{{ __('Full Course') }}</label>
                        @endif
                    </div>
                    <div class="col-xl-6">
                        @if ($course_plan->type == 'prep_course_live')
                            <label class="primary_checkbox nowrap d-flex mr-12">
                                <input type="radio" name="prep_course_live" value="{{ $course_plan->parent_id }}"
                                    id="prep_course_live" class="child_course"
                                    {{ $course_plan->type == 'prep_course_live' ? 'checked' : '' }}>
                                <span class="checkmark mr-2"></span>{{ __('Prep-Course (Live)') }}</label>
                        @endif
                    </div>
                ...
                <div class="row mt-4" id="child_course_price">
                    <div class="col-xl-6">
                        @if ($course_plan->type == 'full_course')
                            <div class="primary_input mb-25" id="full_course_price_div">
                                ...
                                <input class="primary_input_field" accept="/^1[1-9]{9}$/" name="full_course_price"
                                    ...
                                    value="{{ $course_plan->amount ? $course_plan->amount : old('full_course_price') }}"
                    ...
                    <div class="col-xl-6">
                        @if ($course_plan->type == 'prep_course_live')
                            <div class="primary_input mb-25" id="prep_course_live_price_div">
                                <label class="primary_input_label"
                                    for="">{{ __('Prep-Course (Live) Price') }}</label>
                                <input class="primary_input_field" accept="/^1[1-9]{9}$/" name="prep_course_live_price"
                                    placeholder="-" type="text"
                                    value="{{ $course_plan->amount ? $course_plan->amount : old('prep_course_live_price') }}"
                    ...
                    <div class="col-lg-12 pt_15 text-center">
                        <div class="d-flex justify-content-center">
                            <button class="primary-btn semi_large2 fix-gr-bg" id="submit_form">
                                <i class="ti-check"></i>{{ __('Update Course Plan') }}
                            </button>
                        </div>
                    </div>
                </div>
